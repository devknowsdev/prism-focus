...
(function(root){
  ...
  function firstAvailable(preferredKey,fallbackKey){ return getUrl(preferredKey)||getUrl(fallbackKey)||''; }
  ...
  function toolCard({icon,title,status,description,onlineUrl,localUrl,repoUrl,comingSoon}){
    ...
    return `
      <article style="border:1.5px solid ${border};background:${surface};border-radius:14px;padding:14px;display:grid;gap:9px;box-shadow:0 8px 24px rgba(0,0,0,0.08);">
        ...
        <p style="margin:0;color:${dim};font-size:11px;">Preferred mode: ${prismEsc(preferred)}. Missing URLs can be set below once and reused.</p>
      </article>`;
  }
  ...
  function settingsField(key,label,placeholder){
    ...
    const text=prismTheme('text','#1f2937');
    return `<label style="display:block;color:${prismTheme('muted','#6b7280')};font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;">${prismEsc(label)}
      <input data-prism-tools-url-key="${prismEsc(key)}" value="${prismEsc(getUrl(key))}" placeholder="${prismEsc(placeholder||'')}" style="width:100%;margin-top:7px;padding:8px 10px;border-radius:9px;border:1.5px solid ${border};background:${bg};color:${text};font:inherit;font-size:13px;">
    </label>`;
  }
  ...
  function renderSettings(){
    ...
    const preferred=getPreferredMode();
    return `
      <div style="padding:16px 20px;border-bottom:1px solid ${border};background:${surface};display:grid;gap:14px;">
        <div style="display:flex;flex-wrap:wrap;align-items:end;gap:12px;">
          <label style="display:block;color:${muted};font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;">Preferred opening mode
            <select id="prism-tools-preferred-mode" style="display:block;margin-top:7px;padding:8px 10px;border-radius:9px;border:1.5px solid ${border};background:${prismTheme('bg','#f8fafc')};color:${prismTheme('text','#1f2937')};font:inherit;font-size:13px;">
          ...
          <p style="margin:0;color:${muted};font-size:12px;max-width:620px;">Set Cloudflare URLs once. Local URLs stay as fallbacks for branch testing or offline work.</p>
        </div>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:10px;">
          ${settingsField('epkPublisherOnline','EPK Publisher online URL','https://<protected-publisher-url>/publisher/index.html')}
          ${settingsField('epkPublisherLocal','EPK Publisher local URL','http://localhost:8095/publisher/index.html')}
          ${settingsField('epkPublicOnline','EPK public online URL','https://<your-epk-domain>/')}
          ${settingsField('epkPublicLocal','EPK public local URL','http://localhost:8095/')}
          ${settingsField('focusOnline','Focus online URL','https://<protected-focus-url>/')}
          ${settingsField('focusLocal','Focus local URL','http://localhost:8080/')}
          ${settingsField('spectraOnline','Spectra online/local UI URL','')}
          ${settingsField('beamOnline','Beam docs/reference URL','')}
        </div>
      </div>`;
  }
  ...
  function renderPrismToolsModalHtml(){
    ...
    const cards=[
      {
        ...
        onlineUrl:getUrl('epkPublisherOnline'),
      ...
      },
      {
        ...
        onlineUrl:getUrl('epkPublicOnline'),
      ...
      },
      {
        ...
        onlineUrl:getUrl('focusOnline')||currentFocusUrl,
    ...
    return `
      <div data-no-clobber="true" style="position:fixed;inset:0;z-index:9100;background:rgba(0,0,0,0.42);display:flex;align-items:flex-start;justify-content:center;padding:5vh 14px;overflow:auto;">
        <section role="dialog" aria-modal="true" aria-label="Prism Tools" style="width:min(1080px,100%);background:${bg};color:${text};border:1.5px solid ${border};border-radius:18px;box-shadow:0 24px 80px rgba(0,0,0,.32);overflow:hidden;">
          <header style="display:flex;align-items:flex-start;justify-content:space-between;gap:14px;padding:18px 20px;border-bottom:1px solid ${border};background:${surface};">
            <div>
              ...
              <h2 style="margin:0;color:${text};font-size:24px;line-height:1.05;">One workspace, online/local aware.</h2>
              <p style="margin:8px 0 0;color:${muted};font-size:13px;max-width:760px;">Use Cloudflare URLs for normal work and local URLs only for testing branches or offline development.</p>
            </div>
            <button type="button" data-prism-tools-close="true" title="Close Prism Tools" style="${prismBtn('default','padding:7px 10px;border-radius:9px;font-size:13px;')}"><i class="ti ti-x"></i></button>
          </header>

          ${renderSettings()}

          <div style="padding:18px 20px;display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:12px;">
  ...
  function bindModalEvents(){
    document.querySelectorAll('[data-prism-tools-open]').forEach(btn=>{
      btn.addEventListener('click',()=>openExternal(btn.dataset.prismToolsOpen));
    });
